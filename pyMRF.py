...
class Element:
    ...
    def gibbs_sample(self, t, beta_jump_length, verbose, fix_beta, beta_compl_accept, 
        ...
        total_energy_field = self.get_total_energy(dist_KHMD, gibbs_energy)       
    ...
    def fit(self, num_of_iter=100, beta_prior_mean=1, beta_compl_accept=False, fix_beta=False, beta_prior_cov=100,
            beta_init=None, image_init=None, labels_prob_init=None, beta_jump_length=0.01, t=1, verbose=False,
            epsilon = 1, k =None, range_v=None, range_h=None):
            ...
            if self.num_not_fixed > 0:
                if image_init is None:
                    if labels_prob_init is None:
                        if self.mask is None:
                            dist_KHMD, init_labels_prob = self.get_init_labels_prob(epsilon, k, range_v, range_h)                        
                        else:
                            dist_KHMD_inmask, init_labels_prob_inmask = self.get_init_labels_prob(epsilon, k, range_v, range_h)
                            init_labels_prob_outmask = np.ones(shape=(sum(~self.mask), self.n_labels))*(1/self.n_labels)
                            init_labels_prob = np.zeros((self.num_not_fixed, self.n_labels))
                            init_labels_prob[self.mask[~self.fixed_flag],:] = init_labels_prob_inmask
                            init_labels_prob[~self.mask[~self.fixed_flag],:] = init_labels_prob_outmask
                            dist_KHMD = np.zeros((self.num_not_fixed, self.n_labels))
                            dist_KHMD[self.mask[~self.fixed_flag],:] = dist_KHMD_inmask
                            dist_KHMD[~self.mask[~self.fixed_flag],:] = 0                       
                    else:
                        init_labels_prob = labels_prob_init[~self.fixed_flag]
                    self.labels[0][~self.fixed_flag] = draw_labels_vect(init_labels_prob)
                else:
    ...
    def get_DANN_distance(self, epsilon, range_v, range_h):
        ...
        if self.phyDim == 2 :     
            idx_v_pre = np.tile(np.arange(self.phys_shp[0]).reshape((self.phys_shp[0],1)),self.phys_shp[1]).flatten().reshape((self.num_pixels,1))
            idx_h_pre = np.tile(np.arange(self.phys_shp[1]),self.phys_shp[0]).reshape((self.num_pixels,1))
            ...
            coor_BH = np.hstack((idx_h_fixed, idx_v_fixed)) # coordinates of borehole elements
            labels_BH = self.init_field[self.fixed_flag]  # labels of borehole elements
            coor_unknown = np.hstack((idx_h_not_fixed, idx_v_not_fixed))   # coordinates of unknown elements   
            
            distance_DANN = []            
            for q in trange(num_not_fixed):
                coor_unknown_i = coor_unknown[q,:] # coordinate of unknown_i
                ...
                flag_coor_BH = flag_coor_BH_h.copy()
                flag_coor_BH[~flag_coor_BH_v] = False # Marks the borehole elements within the selected rectangle 
                ...
                coor_nhood_BH = coor_BH[flag_coor_BH, :] # coordinates of borehole elements within rectangle(neighborhood) 
                coor_nhood_BH_mean = coor_nhood_BH.mean(axis=0)
                label_nhood_BH = labels_BH[flag_coor_BH]  # labels of borehole elements within rectangle 
                labels_in_nhood = np.unique(label_nhood_BH) # Number of labels within rectangle 
                label_frequencies = {}
                ...
                for target_label in labels_in_nhood:
                    label_indices = np.where(label_nhood_BH == target_label)[0]
                    label_frequencies[target_label] = np.sum(label_nhood_BH == target_label) / sum(flag_coor_BH)
                    label_cov = np.cov(coor_nhood_BH[label_indices, :], rowvar=False)
                    within_label_cov += label_cov * label_frequencies[target_label]
                    label_mean = coor_nhood_BH[label_indices, :].mean(axis=0)
                    between_label_mdiff += np.outer(label_mean - coor_nhood_BH_mean,
                                                  label_mean - coor_nhood_BH_mean) * label_frequencies[target_label]
                negative_flag = within_label_cov < 0
                within_label_cov[negative_flag] = 0
                # W* = W^-.5
                # B* = W*BW*
                W_star = np.linalg.pinv(np.nan_to_num(np.power(within_label_cov, 0.5)))
                B_star = np.dot(W_star, between_label_mdiff).dot(W_star)
                identity_I = np.identity(n_dimension)
                local_matrix = W_star.dot(B_star + epsilon * identity_I).dot(W_star)
                distance_DANN_of_i = []
                for coor_BH_single in coor_BH:
                    # the difference between BH elements in neighborhood and unknown_i
                    difference = coor_unknown_i - coor_BH_single 
                    # the DANN_distance between BH elements in neighborhood and unknown_i
                    distance_DANN_single_of_i = difference.T.dot(local_matrix).dot(difference) 
                    # the DANN_distance of unknown_i
                    distance_DANN_of_i.append(distance_DANN_single_of_i)
                distance_DANN_of_i = np.array(distance_DANN_of_i) 
                # the DANN_distance of all unknown elements 
                distance_DANN.append(distance_DANN_of_i)
    ...
    def get_init_labels_prob(self, epsilon, k, range_v, range_h):
        ...
        if self.phyDim == 2 :                         
            ...
            comp_deck = np.einsum('ijk,i->ijk',np.array([np.ones((num_not_fixed, self.num_fixed)), ]*self.n_labels),np.arange(self.n_labels))
            ...
            distances_elem_sort_desk_for_label = []
            for i in range(self.n_labels):
                a = distances_elem_sort_desk[i,:,:][label_flag[i,:,:]].reshape(((num_not_fixed, sum(label_flag[i,0,:]))))[:,0:k]   
                distances_elem_sort_desk_for_label.append(a)
            distances_elem_sort_desk_for_label = np.array(distances_elem_sort_desk_for_label) 
            zero_flag = distances_elem_sort_desk_for_label==0
            distances_elem_sort_desk_for_label[zero_flag] = 10**(-10)
            ...
            dist_for_label_exp_sum = np.tile(np.sum(dist_for_label_exp, axis=1).reshape((num_not_fixed,1)),self.n_labels)
            init_labels_prob = dist_for_label_exp / dist_for_label_exp_sum
            
            return dist_KHMD, init_labels_prob
...
def save_to_vtk(data, filepath):
    ...
    save the 3d data to a .vtk file in order to visualize it in Paraview
